...
const title='知华访问权限复核', subtitle='权限清单、复核活动、整改跟踪与审计证明';
const token=ref(sessionStorage.getItem('access-review-session')||''),user=ref(null),catalog=ref(null),dashboard=ref(null);
...
async function request(path,{method='GET',body,key,raw=false}={}){
 ...
 if(!r.ok){let m='请求失败';try{m=(await r.json()).message||m;}catch{}if(r.status===401){token.value='';user.value=null;sessionStorage.removeItem('access-review-session');}throw new Error(m);}
...
async function login(){await guarded(async()=>{const r=await request('/auth/login',{method:'POST',body:{username:username.value,password:password.value}});token.value=r.token;user.value=r.user;sessionStorage.setItem('access-review-session',r.token);password.value='';await load();});}
async function logout(){await guarded(async()=>{await request('/auth/logout',{method:'POST',body:{}});token.value='';user.value=null;sessionStorage.removeItem('access-review-session');});}
...
async function exportCsv(){await guarded(async()=>{const blob=await request('/export/'+section.value,{raw:true});download(blob,'access-review-'+section.value+'.csv');});}
...
<template>
 ...
 <div v-else class="app-shell" :style="{'--accent':catalog?.accent}">
  <aside class="sidebar">
   <div class="brand"><span class="brand-mark">知</span><div><strong>{{title.replace('知华','')}}</strong><small>ZHUATECH · ACCESS-REVIEW</small></div></div>
