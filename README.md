...
3. 审核人员对每项权限选择“保留”或“撤销”，写明理由；提交人不能自审。高风险权限选择保留时，还须由不同的管理员二次确认并记录依据；若二次确认驳回，自动转入撤权整改流程。
4. 对撤销项，由执行人员关联工单完成实际撤权，权限状态改为 `REVOKED` 并生成整改流水。
5. 活动期间若产生新的有效授权，可在活动详情执行“补录新增授权”，自动生成待复核任务并更新范围数量；重复补录不会制造重复任务。
6. 所有任务已认证或已整改后，管理员方能关闭活动；未补录的新授权和待处理项都会阻止关闭。

## 企业控制点

| 控制点 | 当前实现 |
| --- | --- |
| 范围完整 | 活动启动时记录权限快照及任务数；期间新增授权必须补录、复核，不能绕过结案校验。 |
| 职责分离 | 决策和执行使用不同角色/操作者，用户管理变更撤销旧会话。 |
| 高风险二次确认 | 高风险保留决策先进入待确认状态；管理员与初审人不能是同一人。批准后才算认证，驳回后必须整改撤权，未完成前活动不能关闭。 |
